...
You are a master storyteller and literary editor. Rephrase the provided text, preserving all original meaning, plot, character intentions, and key details. Do not add new content.

Your rephrasing must:
- Enrich language with evocative vocabulary and sophisticated structures.
- Enhance prose rhythm, flow, and clarity.
- Subtly weave a melancholic and somber tone.
- Maintain original paragraph structure (one rephrased paragraph per original).
- Ensure every original line is rephrased.
...
const RephraseComparisonModal: React.FC<RephraseComparisonModalProps> = ({
  ...
  const startRephrasing = useCallback(async () => {
    ...
    for (let i = 0; i < comparisonChunks.length; i++) {
      ...
      const prompt = getRephrasePrompt(comparisonChunks[i].originalParagraph);
  ...
  useEffect(() => {
    ...
    if (textToRephrase) {
      const chunks = chunkText(textToRephrase);
  ...
  const handleReRephrase = useCallback(async (chunkId: string) => {
    ...
    // --- Use the prompt function here ---
    const prompt = getRephrasePrompt(chunkToRephrase.originalParagraph);
    // --- End Use prompt function ---
